    ...
    module Apple
      class Verification
        ...
        def finish(code:, state:, redirect_uri:)
          ...
          Result.failure(error: :apple_unavailable)
        rescue InvalidEmail, InvalidEnvelope, JWT::DecodeError, KeyError, TypeError
          Result.failure(error: :invalid_apple_authentication)
        end
        ...
        def decode_options
          {
            algorithms: ["RS256"],
            jwks: config.apple_provider_client.jwks,
            iss: ISSUER,
            verify_iss: true,
        ...
        def normalized_email(claims)
          return unless claims["email"]
          raise InvalidEnvelope unless [true, "true"].include?(claims["email_verified"])

          Email.normalize(claims.fetch("email"))
        end
