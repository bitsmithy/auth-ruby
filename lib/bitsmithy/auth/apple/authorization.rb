    ...
    module Apple
      class Authorization
        ...
        def authorization_parameters(redirect_uri, verifier, nonce, state)
          {
            ...
            scope: "email"
          }.merge(ceremony_parameters(verifier, nonce, state))
        end

        def ceremony_parameters(verifier, nonce, state)
          {
            state: state,
            nonce: nonce,
        ...
        def validated_redirect_uri(input)
          ...
          raise ConfigurationError, "Apple redirect_uri must be an absolute HTTPS URL"
